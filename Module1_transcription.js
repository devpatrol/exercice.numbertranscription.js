...
function lire(nombre_chaine = '') {
    ...
    if (checkfloat(liste)) {
        var part_en = partie_entier(liste)
        var part_dec = partie_decimal(liste)
        return (lireNombre(part_en) + " virgule " + lireNombre(part_dec));
    } else {
        nombre = parseInt(nombre_chaine)
        return transcrire(nombre)
    }
}
